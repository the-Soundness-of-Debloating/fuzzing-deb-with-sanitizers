...
function run () {
  ...
  case ${seed_num} in 
    ...
    5)  # -i file (echo "Y")
        touch file
        run_command="{ echo \"Y\" | timeout -k 0.5 0.5 ${bin} < $input; } >/dev/null"
        ;; 
    6) # -r dir
        mkdir dir
        ;;  
    7)  # -i file file2 (echo "Y\nY\n")
        touch file file2
        run_command="{ echo -e \"Y\nY\n\" | timeout -k 0.5 0.5 ${bin} < $input; } >/dev/null"
        ;; 
    8)  # -i file (echo "N")
        touch file
        run_command="{ echo \"N\" | timeout -k 0.5 0.5 ${bin} < $input; } >/dev/null"
        ;; 
    9) # -i file file2 (echo "N\nN\n")
        touch file file2
        run_command="{ echo -e \"N\nN\n\" | timeout -k 0.5 0.5 ${bin} < $input; } >/dev/null"
        ;; 
  esac
...
do
  ...
  case $selected_fuzzer in
    afl)
      crashes_dir=${fuzzed_input_dir}/result_${input_num}/${sanitizer_type}/crashes
      total=`ls $crashes_dir/id* | wc -l`
      input_total=`cat ${fuzzed_input_dir}/result_${input_num}/${sanitizer_type}/fuzzer_stats | grep "corpus_count" | grep -o "[0-9]*"`
      ;;
    symcc) 
      # symcc can only run in nosan mode, but can be verified using each kind of sans.
      crashes_dir=${fuzzed_input_dir}/result_${input_num}/nosan/crashes 
      total=`ls $crashes_dir/id* | wc -l`
      input_total=`cat ${fuzzed_input_dir}/result_${input_num}/nosan/fuzzer_stats | grep "corpus_count" | grep -o "[0-9]*"`
      ;;
    radamsa)
      fuzzed_input=${fuzzed_input_dir}/input${input_num}_fuzzed
      total=500
      input_total=500
      ;;
  esac
  ...
  do
    ...
    afl | symcc)
      ...
      ;;
    radamsa)
      input=${fuzzed_input}/input${input_num}_${cnt}
      ;;
    esac
    run origin
    origin_crash=$?
