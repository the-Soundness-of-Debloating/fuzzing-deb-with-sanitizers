...
## 3 检查 specified_num是否为空，在auto_verify中其是为空的
...
function run () {
  ...
  then
    ...
    echo $crash_loc >> $result_file
  ### tmp不存在，检查 retcode 是否在 130 到 139 的范围内，通常表示由于某种错误导致程序中止
  ...
  then
    ...
    then 
      ...
      find_crash=1
      mv core* core
      gdb $bin core -ex "bt -30" -ex q > tmp # 使用 gdb 分析该核心文件，输出最近的 30 行堆栈跟踪到 tmp 文件中。
      crash_loc=`grep "$program_name.c.${test_type}.c" tmp | grep -o "$program_name.c.${test_type}.c:[0-9]*" | grep -o ":[0-9]*" | grep -o -E "[0-9]*" | tr '\n' ';'`
      echo $crash_loc >> $result_file
    #### 核心文件不存在就没错误
...
function save () {
  input=$1
  crash_loc_array=(${crash_loc//,/ }) # 将crash_loc拆为数组
  ...
  do
    ...
    ### 判断save_dir中是否存在该崩溃位置的目录，如果不存在就创建， 就是说 $crash是唯一判断参数
