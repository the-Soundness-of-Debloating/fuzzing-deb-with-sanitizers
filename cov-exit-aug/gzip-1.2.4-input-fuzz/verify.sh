...
for test_type in ${program_type[@]}; do
  ...
  case $selected_fuzzer in
    afl)
      fuzzed_input_dir=${home_folder}/result/afl_result/reduced
      result_dir=${home_folder}/result/afl_result/analysis/${test_type}/$sanitizer_type
      calc_dir=${home_folder}/result/afl_result/analysis/calc
      ;;
    radamsa)
...
function run () {
  ...
  case ${seed_num} in 
    0) # -c <references/sample1.ref >sample1.gz
      run_command=" { timeout -k 0.5 0.5 ${bin} -c < $input >sample1.gz; } >/dev/null "
      ;;    
    1) # -d <sample1.gz >sample1.tst;
      run_command=" { timeout -k 0.5 0.5 ${bin} -d < $input >sample1.tst; } >/dev/null "
      ;;   
    2) # -f foo
      run_command=" { timeout -k 0.5 0.5 ${bin} -f $input; } >/dev/null "
      ;;   
    3) # -t foo
      run_command=" { timeout -k 0.5 0.5 ${bin} -t $input; } >/dev/null "
      ;;   
    4) # -t sample1.gz
      run_command=" { timeout -k 0.5 0.5 ${bin} -t $input; } >/dev/null "
      ;;   
  ...
  then 
    ...
    elif [ $sanitizer_type == "ubsan" ]; # for ubsan
    then
...
function save () {
  ...
  do
    ...
    then 
      cp $input $crash_dir/id:$(echo $input | grep -oP "(?<=id:)[0-9]+"),input_$seed_num
    else
