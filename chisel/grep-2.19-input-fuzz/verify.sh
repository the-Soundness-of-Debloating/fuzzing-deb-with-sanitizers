...
function run () {
  ...
  0) # "a" input2;
      ...
      ;;
  1) # "a" -v -H -r input_dir
      run_script=" a -v -H -r $input"
      ;;
  2) # "1" -h -r input_dir
      run_script=" 1 -h -r $input"
      ;;
  3) # -n "si" input2
      run_script=" -n si $input"
      ;;
  4) # "1" -r input_dir -l
      run_script=" 1 -r $input -l"
      ;;
  5) # "1" -r input_dir -L
      run_script=" 1 -r $input -L"
      ;;
  6) # "randomtext" -r input_dir -c
      run_script=" randomtext -r $input -c"
      ;;
  7) # -o [r][a][n][d]* input2
      run_script=" -o [r][a][n][d]* $input"
      ;;
  8) # "1" -r input_dir -q
      run_script=" 1 -r $input -q"
      ;;
  9) # "1" -r input_dir -s
      run_script=" 1 -r $input -s"
      ;;
  10) # -v "a" input2
      run_script=" -v a $input"
      ;;
  11) # -i "Si" input2
      run_script=" -i Si $input"
      ;;
  12) # -w "Si" input2
      run_script=" -w Si $input"
      ;;
  13) # -x "Don't" input2
      run_script=" -x Don't $input"
      ;;
  14) # -F "randomtext*" input2
      run_script=" -F randomtext* $input"
      ;;
  15) # -E "randomtext*" input2
      run_script=" -E randomtext* $input"
      ;;
  16) # "ye " input
      run_script=" 'ye ' $input"
      ;;
  17) # "cold" input
      run_script=" cold $input"
      ;;
  18) # "not exist" input
      run_script=" not exist $input"
      ;;
  19) # ^D input2
      run_script=" ^D $input"
      ;;
  ...
  22) # \^$ input2
      ...
      ;;
  23) # ^[AEIOU] input2
      run_script=" ^[AEIOU] $input"
      ;;
  24) # ^[^AEIOU] input2
      run_script=" ^[^AEIOU] $input"
      ;;
  25) # -E "free[^[:space:]]+" input2
      run_script=" -E 'free[^[:space:]]+' $input"
      ;;
  26) # -E '\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' input
      run_script=" -E '\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' $input"
      ;;
  esac
